...
3. Подключите репозиторий GitHub:
   ...
   - Ветка: `main`
4. **Путь к Dockerfile в репозитории:**
   - Timeweb автоматически ищет Dockerfile в корне
   - Используйте файл: **`Dockerfile.backend`**
   - ⚠️ **ВАЖНО:** При создании приложения укажите имя, например: `nasledniki-backend`
5. **Регион:** Россия / Санкт-Петербург
6. **Конфигурация:** 
   - Фиксированная: 1 CPU, 2 ГБ RAM, 30 ГБ NVMe (~810 ₽/мес)
   - **Рекомендуется:** 1 × 3.3 ГГц, 2 ГБ RAM, 30 ГБ (~810 ₽/мес)
